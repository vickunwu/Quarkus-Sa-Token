package io.vickunwu.external.spring.utils;
...
public class AntPathMatcher implements PathMatcher {
    ...
    public String combine(String pattern1, String pattern2) {
        if (!StrUtil.isNotBlank(pattern1) && !StrUtil.isNotBlank(pattern2)) {
            return "";
        }
        if (!StrUtil.isNotBlank(pattern1)) {
            return pattern2;
        }
        if (!StrUtil.isNotBlank(pattern2)) {
            return pattern1;
        }
    ...
    public static String[] tokenizeToStringArray(
        ...
        return CollUtil.isNotEmpty(tokens) ? tokens.toArray(EMPTY_STRING_ARRAY) : EMPTY_STRING_ARRAY;
    }
